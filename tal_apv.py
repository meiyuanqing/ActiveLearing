...
def active_learning_mvs_middle(working_dir, result_dir):
    ...
    dir_spv = result_dir + 'apv_5_percent/'
            ...
            if name == versions_sorted[len(versions_sorted) - 1]:
                ...
                for i in range(percent_5):
                    ...
                    for metric in metrics_20:
                        ...
                        df_previous_all = df_previous_all[
                            ~df_previous_all[metric].isin(['undef', 'undefined'])].reset_index(drop=True)
                        df_previous_all = df_previous_all[~df_previous_all[metric].isnull()].reset_index(drop=True)
                        df_previous_all = df_previous_all[~df_previous_all['loc'].isin([0])].reset_index(drop=True)
                    ...
                    for row in range(len(df_name)):
                        if df_name.loc[row, 'pred_20_score'] == score_list[math.ceil(len(score_list) * 0.5)]:
                            ...
                            break
                    # print(df_name.pred_20_score.median())
                    # print(df_name)
                    # print(df_name[df_name['iter'] == 1])
                    ...
                    # 用之前所有版本真实标签代入当前版本训练阈值
                    df_previous_all['iter_bug'] = df_previous_all.bug.apply(lambda x: 1 if x > 0 else 0)
                    ...
                    df_training = df_training.append(df_manual_labels.loc[:, metrics_20 + ['iter_bug']]).reset_index(
                        drop=True)
                    df_training = df_training.append(df_previous_all.loc[:, metrics_20 + ['iter_bug']]).reset_index(
                        drop=True)
