...
File: RQ3_TAL_min_prior_pseudo_loc_statistic_spv.py
Date : 2022/11/24 17:11
Author : njumy
HomePage : https://github.com/meiyuanqing
Email : dg1533019@smail.nju.edu.cn
与RQ3_TAL_middle_prior_pseudo_loc_statistic_spv.py相似，但是取投票得分最小值的模块用于主动学习
与tal_spv_test.py代码相同，再增加每次选出主动学习模块并保存，同时再增加一张表，用于保存31个版本上，（spv有21个，apv有10个）
选出模块的SLOC信息，包括最大值，最小值，平均值，总和，方差五个信息，最后可能将平均值和总和用于wilcoxon和cliff的比较。
...
def active_learning_mvs_middle(working_dir, result_dir):
    ...
    # 存储选出模块的SLOC信息，包括最大值，最小值，平均值，总和，方差五个信息，最后可能将平均值和总和用于wilcoxon和cliff的比较
    df_loc_statistic = pd.DataFrame(columns=['Sample_size', 'project', 'last_version', 'current_version',
                                             'loc_max', 'loc_min', 'loc_avg', 'loc_sum', 'loc_var'], dtype=object)
        ...
        for name in versions_sorted:
            ...
            for i in range(percent_5):
                ...
                for row in range(len(df_name)):
                    if df_name.loc[row, 'pred_20_score'] == score_list[-1]:
                ...
                df_test.to_csv(dir_spv + 'testing_data_' + name, index=False)

                # 每次迭代时，对上次未能被选中的模块V-score得分清零，
                df_name['pred_20'] = df_name.apply(lambda x: 0 if x['iter'] == 0 else x['pred_20'], axis=1)

            df_iter.to_csv(dir_spv + 'active_learning_data_' + name, index=False)

            # store loc statistic
            df_loc_statistic = df_loc_statistic.append(
                {'Sample_size': len(df_iter), 'project': project,
                 'last_version': name_previous[:-4], 'current_version': name[:-4], 'loc_max': df_iter['loc'].max(),
                 'loc_min': df_iter['loc'].min(), 'loc_avg': df_iter['loc'].mean(), 'loc_sum': df_iter['loc'].sum(),
                 'loc_var': df_iter['loc'].var()}, ignore_index=True)
            df_loc_statistic.to_csv(dir_spv + 'loc_statistic.csv', index=False)
...
if __name__ == '__main__':
    ...
    result_Directory = "/home/myq/talcvdp/active_learning_min_prior_pseudo_loc_statistic/"
