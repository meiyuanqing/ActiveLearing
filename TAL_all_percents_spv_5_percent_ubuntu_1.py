...
File: RQ3_TAL_all_percents_spv.py
Date : 2022/11/16 16:00
Author : njumy
HomePage : https://github.com/meiyuanqing
Email : dg1533019@smail.nju.edu.cn

For the first stage, the module with the median value of the voting measure V(xi) is selected as the candidate module.
    In this question, we investigate whether the other percentile values of the voting measure impact our experimental
    results. We select the following ten percentiles of V-score: 10%, 20%, 25%, 30%, 40%, 60%, 70%, 75%, 80%, 90%
    as the baseline methods to conduct the TAL experiments.
    We name this baseline method as TAL-percent, such as TAL-10%.
...
def active_learning_mvs_middle(working_dir, result_dir):
    ...
    dir_spv = result_dir + 'spv_5_percent/TAL_1_percent/'

    if not os.path.exists(result_dir):
        os.mkdir(result_dir)

    if not os.path.exists(result_dir + 'spv_5_percent/'):
        os.mkdir(result_dir + 'spv_5_percent/')
...
if __name__ == '__main__':
    ...
    result_Directory = "/home/myq/talcvdp/active_learning_middle_prior_pseudo_all_percents/"
