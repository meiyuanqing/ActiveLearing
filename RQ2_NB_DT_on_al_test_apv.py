...
File: RQ2_NB_DT_SVM_on_al_test_apv.py
...
def random_forest_on_tal_test(working_dir, result_dir):
    ...
    dir_spv_dt = result_dir + 'DT/apv_20_percent/'
    dir_spv_svm = result_dir + 'svm/apv_20_percent/'
    ...
    for project_t in projects_t:
        ...
        for name in versions_sorted:
            ...
            if name == versions_sorted[len(versions_sorted) - 1]:
                ...
                gnb = GaussianNB()
                df_name_test['predictBinary'] = gnb.fit(df_previous_all.loc[:, metrics_20],
                                                        df_previous_all.loc[:, 'bugBinary']).predict(
                    df_name_test.loc[:, metrics_20])
                ...
                clf_dt = clf_dt.fit(df_previous_all.loc[:, metrics_20], df_previous_all.loc[:, 'bugBinary'])
                ...
                # svm
                clf_svm = svm.SVC()
                clf_svm.fit(df_previous_all.loc[:, metrics_20], df_previous_all.loc[:, 'bugBinary'])
